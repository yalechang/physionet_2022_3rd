...
class MultiTaskClassifierWithMFCC(nn.Module):
    def __init__(self, dim_pcg=1, seq_len=3000, dim_mfcc=1, len_mfcc=3000,# input signal
        num_layer_shared=1, num_kernels_shared=50, kernel_size_shared=15, # shared layers
        dim_static=10, dim_embedding=128, # intermediate layer
        num_layer_task=1, num_kernels_task=50, kernel_size_task=3, # task-specific layers 
        n_class_first_task=3, n_class_second_task=2): # output classes
        ...
        dim_pcg: int, dimension (# channels) of input heart sound recording
        seq_len: length of the input heart sound recording sequence
        dim_mfcc: int, dimension of the MFCC features
        len_mfcc: int, length of the MFCC features
        ...
        n_class_second_task: int, the output class count of the second task
        """
        super(MultiTaskClassifierWithMFCC, self).__init__()

        # shared layer to process PCG
        self.shared_layer_pcg = nn.ModuleList()
        # construct shared layers
        self.num_layer_shared = num_layer_shared
        for i in range(self.num_layer_shared):
            if i == 0:
                ...
                in_channel = num_kernels_shared
            # each layer consists of: conv1d + batchNorm + ReLU
            self.shared_layer_pcg.append(nn.Sequential(\
                nn.Conv1d(in_channel, num_kernels_shared, kernel_size=kernel_size_shared, 
                            padding=(kernel_size_shared-1)//2), 
                nn.BatchNorm1d(num_kernels_shared), \
                nn.ReLU()))
        ...
        # add embedding layer: get embedding of size dim_embedding and apply batch normalization
        self.embedding_layer_pcg = nn.Linear(num_kernels_shared*seq_len, dim_embedding)
        self.batch_norm_pcg = nn.BatchNorm1d(dim_embedding)

        # shared layer to process MFCC
        self.shared_layer_mfcc = nn.ModuleList()
        for i in range(self.num_layer_shared):
            if i == 0:
                # equal to input dimenion
                in_channel = dim_mfcc
                ...
                in_channel = num_kernels_shared
            # each layer consists of: conv1d + batchNorm + ReLU
            self.shared_layer_mfcc.append(nn.Sequential(\
                nn.Conv1d(in_channel, num_kernels_shared, kernel_size=kernel_size_shared, 
                            padding=(kernel_size_shared-1)//2), 
                nn.BatchNorm1d(num_kernels_shared), \
                nn.ReLU()))
        ...
        # add embedding layer: get embedding of size dim_embedding and apply batch normalization
        self.embedding_layer_mfcc = nn.Linear(num_kernels_shared*len_mfcc, dim_embedding)
        self.batch_norm_mfcc = nn.BatchNorm1d(dim_embedding)        
        ...
        # output layers
        self.linear_first_task = nn.Linear((dim_embedding*2+dim_static)*num_kernels_task, n_class_first_task)
        self.linear_second_task = nn.Linear((dim_embedding*2+dim_static)*num_kernels_task, n_class_second_task)
    ...
    def forward(self, x_pcg, x_mfcc, x_static):
        """
        x_pcg: N x C_in x L_in:
        x_mfcc: N x C_in x L_in:
        x_static: N x dim_static
        """
        # pass x_pcg into shared layer
        for i, layer in enumerate(self.shared_layer_pcg):
            x_pcg = layer(x_pcg)
        # pass x_mfcc into shared layer
        for i, layer in enumerate(self.shared_layer_mfcc):
            x_mfcc = layer(x_mfcc)
        
        # reshape to N x (num_kernels_shared x seq_len) and apply embedding layer followed by batch normalization
        x_embd_pcg = self.embedding_layer_pcg(x_pcg.view(x_pcg.shape[0], -1))
        x_embd_pcg = self.batch_norm_pcg(x_embd_pcg)

        x_embd_mfcc = self.embedding_layer_mfcc(x_mfcc.view(x_mfcc.shape[0], -1))
        x_embd_mfcc = self.batch_norm_mfcc(x_embd_mfcc)       

        # concatenate x_embd with x_static
        x_embd = torch.cat((x_embd_pcg, x_embd_mfcc, x_static), 1)
        #print("x_embed: ", x_embd.shape)
        # apply task-specific layer
        x_first = x_embd.view(x_embd.shape[0], 1, x_embd.shape[1])
        for layer in self.layer_first_task:
